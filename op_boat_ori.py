...
def update_BoatStatus(boats,berths,capacity,now_frame):
    ...
    for index_boat in range(5):
        ...
        if now_frame < 6000 and target_table_boat_berth[index_boat][2] < 2:
            if target_table_boat_berth[index_boat][1] == 0:
                    ...
                    #如果是第二轮要用下面的选
                    elif target_table_boat_berth[index_boat][2] == 1:
                    ...
                    #没装满。考虑两种情况，第一种是第一轮能转2次，第二种是第二轮只能转一次
                        ...
                        #如果是第二轮只能转1次
                            ...
                            #第二轮装货状态6000帧必须到虚拟点
                                ...
                                if (6000 - now_frame) <= berths[index_berth].transport_time:
                                    target_table_boat_berth[index_boat][1] = 4
                    #进入阶段2  
        else:
            #全新改版
            if 6000<= now_frame <= 6005 or 9000 <= now_frame <= 9005 or 12000 <= now_frame <= 12005 :
                if target_table_boat_berth[index_boat][1] == 0:
                #只有最后一轮直接定
                        berth_index = select(berths,capacity)
                        target_table_boat_berth[index_boat][0] = berth_index
...
def select_first_two_first(berths,now_frame):
    ...
    for i in range(10):
        if berths[i].choosen == 1:
            ...
            time_left = 6000 - 2 * berths[i].transport_time - now_frame - load_time
            if time_left < 0:
                metric[i] = -1
            else:
...
def select_first_two_second(berths,boat_v_left,now_frame):
    ...
    for i in range(10):
        if berths[i].choosen == 1:
            ...
            time_of_you = 5500 - now_frame  - berths[i].transport_time
            if time_of_you <= 0:
                metric[i] = -1
            else:
